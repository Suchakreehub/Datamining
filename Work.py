...
class NextPage(tk.Tk):
    ...
    def display_results(self):
        ...
        fawfa = display_min_fitness_chromosomes
...
def display_min_fitness_chromosomes(self, random_data_gene):
        ...
        # เก็บ Chromosomes ที่มีค่า Fitness ต่ำสุด
        x, y = [], []
        ...
        # แสดง Chromosomes ที่มีค่า Fitness ต่ำสุด
        count = 0
        for i, chromosome in enumerate(random_data_gene):
            if chromosome[4] in min_fitness_values:
                print(f"Chromosomes {i+1}: Gender: {chromosome[0]}, Age: {chromosome[1]}, Workout(Min): {chromosome[2]}, Smoking: {chromosome[3]}, Fitness {chromosome[4]}%")
                count += 1
            if count == 2:
                break
